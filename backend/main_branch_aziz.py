...
def get_user_favorites(
    ...
    """Returns the user's top rated movies to seed the 'Because you liked...' rows."""
    # Ensure they can only fetch their own favorites or allow it generally if user_id matches
    if user_id != current_user.id and current_user.id != 1: # Basic check
        uid_to_fetch = current_user.id
    else:
        uid_to_fetch = user_id
    ...
    query = text("""
        ...
        LIMIT :limit
    """)
    rows = db.execute(query, {"uid": uid_to_fetch, "limit": limit}).fetchall()
    
    return {
        "user_id": uid_to_fetch,
        "favorites": [
            {
                "movie_id": r.movie_id,
                "title": r.title,
...
@app.get("/api/recommendations/group")
def get_group_recommendations_least_misery(
    user_ids: str = Query(..., description="Comma-separated PostgreSQL user ids (min 2), must include the caller"),
    ...
    """
    Group recommendations using **least misery**: for each candidate movie, take the
    SVD predicted rating (1–5 scale) for every group member; the group's score is
    the **minimum** of those predictions — so one strong dislike caps the whole group.

    Returns the top `limit` movies (default 20) by `group_score` descending.
    Candidates are movies none of the group has rated yet, with enough community ratings.
    """
    svd = ml_artifacts.get("svd_model")
    if not svd:
        raise HTTPException(status_code=503, detail="Service temporarily unavailable.")
    ...
    if current_user.id not in group_ids:
        raise HTTPException(
            status_code=403,
            detail="You must include your own user id in user_ids.",
        )
    ...
    for cr in c_rows:
        ...
        scored.append({
            ...
            "reason": (
                f"Least misery: min predicted rating across the group is {round(group_score, 2)}/5 — "
                "no one is predicted below this."
            ),
        })
...
def get_trending_by_genre(
    ...
    """
    Trending movies grouped by genre, personalized by the current user's
    "liked" history (ratings >= 4.0).

    Genres are ranked by how many movies the user liked in each genre.
    Within each genre, movies are ranked by community "trending_score".
    """
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    # Send welcome email in a fire-and-forget background thread so an SMTP
    # timeout (can be 30-60 s) never blocks or delays the registration response.
    threading.Thread(
        target=lambda: __import__('asyncio').run(send_welcome_email(new_user.email, new_user.username)),
        daemon=True
    ).start()
...
# ==============================================================================
# WATCH TOGETHER — Tinder-style swipe-to-match feature
# ==============================================================================
#
# Flow:
#   1. User A calls POST /api/sessions            → gets back a session code
#   2. User A shares the code (or invite link) with User B
#   3. User B calls POST /api/sessions/{code}/join
#   4. Both users poll GET /api/sessions/{code}   → status flips "waiting"→"active"
#   5. Both users call POST /api/sessions/{code}/swipe for each card
#   6. On a mutual right-swipe the endpoint returns {"match": true} and stores
#      the match so the frontend can trigger the "IT'S A MATCH!" animation.
#   7. GET /api/sessions/{code}/matches returns the full list at any time.
#
# Real-time (WebSocket) broadcast is in the next section below.
# ==============================================================================

import random as _random
import string as _string

# ── Helper: 8-char uppercase invite code ──────────────────────────────────────

def _generate_session_code(length: int = 8) -> str:
    """e.g. 'FILM4829'  — uppercase letters + digits, easy to read aloud."""
...
@app.post("/api/sessions", summary="Create a Watch-Together session")
def create_session(
    ...
    Pulls a personalised movie pool for the creator using the SVD model
    (same logic as /api/recommendations/top-picks, limited to `pool_size`).
    The pool is stored in the session so both users see the same card stack.

    Returns the session code that User B needs to join.
    """
    svd = ml_artifacts.get("svd_model")
    if not svd:
        raise HTTPException(status_code=503, detail="ML model unavailable — try again later.")

    # Build a simple quality-ordered candidate pool for the creator.
    # When User B joins their preferences will also be factored via the swipe
    # scoring — fetching the full group recommendations at join time would add
    # latency that harms UX, so we start with a high-quality general pool.
    ...
    # Persist the session with a unique code (retry on collision — extremely rare)
    for _ in range(5):
        code = _generate_session_code()
        existing = db.execute(
            text("SELECT id FROM watch_sessions WHERE code = :code"),
            {"code": code},
        ).fetchone()
        if not existing:
            break
...
@app.post("/api/sessions/{code}/join", summary="Join an existing Watch-Together session")
def join_session(
    ...
    """
    Lets a second user join a session created by someone else.
    Status switches from 'waiting' to 'active', which both clients detect
    via polling GET /api/sessions/{code}.
    """
    session = db.execute(
    ...
    if session.status != "waiting":
        raise HTTPException(
            status_code=409,
            detail="This session is already active or has ended.",
        )
...
@app.get("/api/sessions/{code}", summary="Poll session status and get the movie pool")
def get_session(
    code: str = Path(..., min_length=1),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    ...
    Clients poll this every 2-3 seconds while status == 'waiting'.
    Once status == 'active', both sides start the swipe UI.
    """
    session = db.execute(
        text("""
            SELECT id, code, creator_id, guest_id, status, movie_pool, created_at, expires_at
            FROM watch_sessions
            WHERE code = :code
        """),
        {"code": code},
    ...
    # Only members of the session may read it
...
@app.post("/api/sessions/{code}/swipe", summary="Record a swipe and detect matches")
def record_swipe(
    code: str = Path(..., min_length=1),
    ...
    """
    Records one swipe (left or right) for the authenticated user on a given movie.

    **Match detection** — if the current swipe is 'right' and the *other* user
    has already swiped 'right' on the same movie, a match row is inserted and
    `{"match": true}` is returned so the frontend can fire the animation.

    Duplicate swipes (same user, same movie) are silently ignored and return
    the original result so clients can retry safely.
    """
    session = db.execute(
        text("SELECT id, creator_id, guest_id, status FROM watch_sessions WHERE code = :code"),
        {"code": code},
    ...
    # Upsert the swipe (ignore if already exists — idempotent endpoint)
    db.execute(
        text("""
    ...
    if body.direction == "right":
        # Check if the other user already swiped right on the same movie
        ...
        if other_swipe:
            # IT'S A MATCH — persist it (ignore if already exists)
            db.execute(
                text("""
            ...
            # Fetch movie details to return with the match payload
            movie_row = db.execute(
...
@app.get("/api/sessions/{code}/matches", summary="Get all matched movies in a session")
def get_session_matches(
    ...
    """
    Returns the full list of movies both users swiped right on.
    Useful for building a shared watchlist at the end of the swipe session.
    """
    session = db.execute(
